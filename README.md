...
Projeto de estudo focado na prática de **CSS Flexbox**, desenvolvido como exercício para consolidar os principais conceitos de layout com `display: flex`. O layout é **responsivo**, com uma versão mobile-first e ajustes para telas desktop via media query.
...
└── styles/
    ...
    ├── utility.css     # Classes utilitárias (ex: .desktop-only, .flex) usadas na responsividade
...
O layout é construído mobile-first e ganha ajustes para telas maiores a partir do breakpoint `@media (width >= 80em)`, aplicado em `global.css`, `nav.css`, `header.css`, `main.css`, `footer.css` e `utility.css`. Entre os ajustes:

- **Nav**: menu e busca ficam ocultos por padrão no mobile e aparecem no desktop através da classe utilitária `.desktop-only` (definida em `utility.css`).
- **Header**: reorganização do perfil e das estatísticas (`#info`) para melhor aproveitamento do espaço em telas pequenas.
- **Galeria (`main`)**: ajuste do `flex-wrap` e do tamanho das fotos para se adaptarem a diferentes larguras de tela.
- **Footer**: empilhamento dos itens no mobile, voltando ao layout em linha no desktop.

## Tecnologias

- HTML5
- CSS3 (Flexbox, variáveis CSS, `@import`, Media Queries)
